...
class InterHandLoader:
    def __init__(self, data_path, split="train", mano_path=None):
        ...
        with open(
            osp.join(
                self.annot_root_path,
                self.split,
                "InterHand2.6M_" + self.split + "_data.json",
            )
        ) as f:
        ...
        with open(
            osp.join(
                self.annot_root_path,
                self.split,
                "InterHand2.6M_" + self.split + "_camera.json",
            )
        ) as f:
        ...
        with open(
            osp.join(
                self.annot_root_path,
                self.split,
                "InterHand2.6M_" + self.split + "_joint_3d.json",
            )
        ) as f:
        ...
        with open(
            osp.join(
                self.annot_root_path,
                self.split,
                "InterHand2.6M_" + self.split + "_MANO_NeuralAnnot.json",
            )
        ) as f:
    ...
    def load_mano(self, idx):
        ...
        for hand_type in ["left", "right"]:
            try:
                ...
                handV, handJ = self.mano_layer[hand_type](
                    root_pose, hand_pose, shape, trans=trans
                )
                mano_dict[hand_type] = {
                    "R": root_pose.numpy(),
                    "pose": hand_pose.numpy(),
                    "shape": shape.numpy(),
                    "trans": trans.numpy(),
                }
        ...
        if hand_type == "interacting" and hand_type_valid:
            ...
            if coord_dict["left"] is not None and coord_dict["right"] is not None:
                ...
                if dist < 9999999:
                    ...
                    [img], _, cameraIn = cut_img(
                        [img],
                        [left2d, right2d],
                        camera=cameraIn,
                        radio=HAND_BBOX_RATIO,
                        img_size=IMG_SIZE,
                    )
                    cv.imwrite(
                    ...
                    with open(
                        osp.join(save_path, split, "anno", "{}.pkl".format(idx)), "wb"
                    ) as file:
...
def render_data(save_path, split):
    ...
    for idx in tqdm(range(size)):
        with open(
            osp.join(save_path, split, "anno", "{}.pkl".format(idx)), "rb"
        ) as file:
        ...
        for hand_type in ["left", "right"]:
            ...
            handV, handJ = mano_layer[hand_type](
                torch.from_numpy(params["R"]).float(),
            ...
            for hIdx in range(len(hms)):
                cv.imwrite(
                    os.path.join(
                        ...
                        split,
                        "hms",
                        "{}_{}_{}.jpg".format(idx, hIdx, hand_type),
                    ),
                    hms[hIdx].transpose(1, 2, 0) * 255,
                )

        img_mask = renderer.render_mask(
            cameras=torch.from_numpy(camera).float().cuda().unsqueeze(0),
            v3d_left=verts[0],
            v3d_right=verts[1],
        )
        ...
        img_dense, _ = renderer.render_densepose(
            cameras=torch.from_numpy(camera).float().cuda().unsqueeze(0),
            v3d_left=verts[0],
            v3d_right=verts[1],
        )
...
class InterHand_dataset:
    ...
    def __getitem__(self, idx):
        img = cv.imread(
            osp.join(self.data_path, self.split, "img", "{}.jpg".format(idx))
        )
        mask = cv.imread(
            osp.join(self.data_path, self.split, "mask", "{}.jpg".format(idx))
        )
        dense = cv.imread(
            osp.join(self.data_path, self.split, "dense", "{}.jpg".format(idx))
        )

        with open(
            os.path.join(self.data_path, self.split, "anno", "{}.pkl".format(idx)), "rb"
        ) as file:
            data = pickle.load(file)
